...
<body style="background-color: yellowgreen;">
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add'])) {
        ...
        $stmt = $connection->prepare("INSERT INTO clients (client_id, trainer_id, start_date, end_date) VALUES (?, ?, ?, ?)");
        $stmt->bind_param("iiss", $client_id, $trainer_id, $start_date, $end_date); // Bind parameters

        // Execute the statement and check for success
        if ($stmt->execute()) {
            echo "New record has been added successfully.<br><br><a href='clients.php'>Back to Form</a>";
        } else {
            ...
            if ($result->num_rows > 0) {
                ...
                while ($row = $result->fetch_assoc()) {
                    echo "<tr>
                            ...
                            <td>{$row['end_date']}</td>
                            <td><a style='padding:4px' href='delete_clients.php?client_id={$row['client_id']}'>Delete</a></td> 
                            <td><a style='padding:4px' href='update_clients.php?client_id={$row['client_id']}'>Update</a></td> 
                          </tr>";
                }
            } else {
