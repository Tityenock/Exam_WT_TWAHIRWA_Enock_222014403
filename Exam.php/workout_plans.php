...
<head>
  ...
  <title>Workout Plans Form</title>
...
<body style="background-color: yellowgreen;">
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add'])) {
      ...
      $stmt = $connection->prepare("INSERT INTO workout_plans (plan_id, trainer_id, plan_name, description, duration, price) VALUES (?, ?, ?, ?, ?, ?)");
      $stmt->bind_param("iisssi", $plan_id, $trainer_id, $plan_name, $description, $duration, $price); // Bind parameters

      // Execute the statement and check for success
      if ($stmt->execute()) {
          echo "New record has been added successfully.<br><br><a href='workout_plans.php'>Back to Form</a>";
      } else {
  ...
  <section>
    ...
    <table>
      ...
      <?php
      // Select all workout plans from the database
      ...
      if ($result->num_rows > 0) {
          ...
          while ($row = $result->fetch_assoc()) {
              echo "<tr>
                      ...
                      <td>{$row['duration']}</td>
                      <td>{$row['price']}</td>
                      <td><a style='padding:4px' href='delete_workout_plans.php?plan_id={$row['plan_id']}'>Delete</a></td> 
                      <td><a style='padding:4px' href='update_workout_plans.php?plan_id={$row['plan_id']}'>Update</a></td> 
                    </tr>";
          }
      } else {
