...
<body style="background-color: yellowgreen;">
  ...
  <h1>Client Goals Form</h1>
  <form method="post" onsubmit="return confirmInsert();">
    <label for="goal_id">Goal ID:</label>
    <input type="number" id="goal_id" name="goal_id" required><br><br>
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['add'])) {
      ...
      $stmt = $connection->prepare("INSERT INTO client_goals (goal_id, client_id, goal_description, target_date) VALUES (?, ?, ?, ?)");
      $stmt->bind_param("iiss", $goal_id, $client_id, $goal_description, $target_date); // Bind parameters

      // Execute the statement and check for success
      if ($stmt->execute()) {
          echo "New record has been added successfully.<br><br><a href='client_goals.php'>Back to Form</a>";
      } else {
  ...
  <section>
    ...
    <table>
      ...
      <?php
      // Select all client goals from the database
      $sql = "SELECT * FROM client_goals";
      ...
      if ($result->num_rows > 0) {
          ...
          while ($row = $result->fetch_assoc()) {
              echo "<tr>
                      <td>{$row['goal_id']}</td>
                      <td>{$row['client_id']}</td>
                      <td>{$row['goal_description']}</td>
                      <td>{$row['target_date']}</td>
                      <td><a style='padding:4px' href='delete_client_goals.php?goal_id={$row['goal_id']}'>Delete</a></td> 
                      <td><a style='padding:4px' href='update_client_goals.php?goal_id={$row['goal_id']}'>Update</a></td> 
                    </tr>";
          }
      } else {
