<?php
    ...
    if(isset($_REQUEST['plan_id'])) {
        ...
        $stmt = $connection->prepare("SELECT * FROM workout_plans WHERE plan_id=?");
...
<body><center>
    ...
    <h2><u>Update Form of Workout Plan</u></h2>
...
if(isset($_POST['up'])) {
    ...
    // Update the workout plan record in the database
    $stmt = $connection->prepare("UPDATE workout_plans SET trainer_id=?, plan_name=?, description=?, duration=?, price=? WHERE plan_id=?");
    $stmt->bind_param("issdii", $trainer_id, $plan_name, $description, $duration, $price, $plan_id);
    $stmt->execute();
    
    // Redirect to workout_plans.php or any other page displaying workout plan records
